...
# ======== feedparser بـ timeout (15 ثانية) لتجنب التعليق اللانهائي ========
def _parse_feed(url, timeout=15):
    result = [None]
    def _fetch():
        try:
...
def _db_save_user(uid, user_data):
    d = dict(user_data)
    if "sent_news" in d:
        d["sent_news"] = list(d["sent_news"])[-120:]
...
DEFAULT_RSS = {
    "العربية 🇮🇶": [
        ...
        "https://www.alsumaria.tv/Rss/News/ar/",
        ...
        "https://ina.iq/rss_feed.xml",
        "https://buratha.com/rss.xml",
        "https://www.mawazin.net/rss.xml",
        "https://non14.net/services/rss",
        "https://aliraqnews.com/feed",
        "https://kitabat.com/feed",
        "https://lvinpress.com/feed",
        "https://xebat.net/ku/?feed=rss2",
        "https://nasiriyah.org/ar/rss",
        "https://azzaman.com/rss",
        "https://almasalah.com/rss",
        "https://basnews.com/en/rss",
        "https://alsharqiya.com/ar/rss",
        "https://almadapaper.net/rss",
        "https://sotaliraq.com/rss",
        "https://imn.iq/feed",
        "https://mangish.net/feed",
        # قنوات عربية كبرى
        ...
        "https://www.asharq.com/feed/",
        "https://www.raialyoum.com/?feed=rss2",
        "https://noonpost.com/feed/",
        "https://arabicpost.net/feed/",
        # مصر
        "https://rss.almasryalyoum.com/rss.xml",
        "https://www.youm7.com/rss/rss.aspx",
        # الخليج
    ...
    "Türkçe 🇹🇷": [
        ...
        "https://www.milliyet.com.tr/rss/rssNew",
    ...
    "Italiano 🇮🇹": [
        ...
        "https://www.rainews.it/dl/rainews/media/feed-xml-feedRainews.xml",
        "https://www.sky.it/feed",
    ],
    "Español 🇲🇽": [
...
def format_news_item(prefix, title, lang=None, source_name=None):
    ...
    if source_name:
        ...
        source_line = ""
    return f"{label}\n\n📰 {title}{source_line}{get_news_signature(lang)}"
    ...
    try:
        ...
        for uid, info in list(users.items()):
            try:
                ...
                feeds = RSS.get(lang, [])
                sent = info.setdefault("sent_news", set())
                # تقليص sent_news تلقائياً إذا كبر كثيراً (يسبب توقف البث)
                if len(sent) > 100:
                    trimmed = set(list(sent)[-60:])
                    info["sent_news"] = trimmed
                    sent = trimmed
                changed = False
                user_count = 0
                for feed_url in feeds:
                    if user_count >= MAX_NEWS_PER_BROADCAST:
                        break
                    if feed_url not in rss_cache:
                        try:
                            rss_cache[feed_url] = _parse_feed(feed_url)
...
def auto_clean_sent_news():
    ...
    for uid, info in users.items():
        ...
        if len(sent) > 80:
            sent_list = list(sent)
            users[uid]["sent_news"] = set(sent_list[-60:])
            cleaned += 1
    if cleaned > 0:
        _db_save_all_users(users)
