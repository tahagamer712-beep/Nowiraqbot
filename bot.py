...
HELP_TEXTS = {
    ...
    "English 🇬🇧": (
        ...
        "/reset — Reset the bot and start over\n"
...
def change_welcome_step(message):
    ...
    if message.text.strip() == "افتراضي":
        ...
        bot.send_message(message.from_user.id, "✅ تم الرجوع لرس�الة الترحيب الافتراضية.")
    else:
 ...
 �       "Türkçe 🇹🇷":   f"👋 *Tekrar hoş geldiniz, {name}!*\n\nSizi yeniden görmekten mutluyuz! 🌍\n\n",
    ...
    body_lines = {
        ...
        "Türkçe 🇹🇷": (
            ...
            "🔄 /reset — Bot kurulumunu yeniden başlat"
        ),
        "اردو 🇵🇰": (
            "📰 لائیو عالمی خبریں\n"
            "🌤 آپ کے شہر کا موسم\n"
            ...
            "🔄 /reset — بوٹ سیٹ اپ دوبارہ شروع کریں"
        ),
        "Deutsch 🇩🇪": (
            ...
            "🔄 /reset — Bot-Einrichtung neu starten"
        ),
        "Українська 🇺🇦": (
            ...
            "🔄 /reset — Перезапустити налаштування"
        ),
        "Italiano 🇮🇹": (
            ...
            "🔄 /reset — Ricomincia la configurazione"
        ),
        "Español 🇲🇽": (
            ...
            "🔄 /reset — Reiniciar configuración del bot"
        ),
    }
    contact_labels = {
        "العربية 🇮🇶": "للتواصل",
...
MSGS = {
    "العربية 🇮🇶": {
        ...
        "premium_btn_weekly": "📰 م�لخص أسبوعي",
    ...
    "English 🇬🇧": {
        ...
        "dollar_sell": "Sell",
        "dollar_buy": "Buy",
        "dollar_official": "Rate: `{price}` IQD\n_(Official rate — may diff�er from market)_",
...
�        "currency_unsupported": "⚠️ Desteklenmeyen para birimi: {currency}",
    ...
    "اردو 🇵🇰": {
        ...
        "t�rack_alert_title": "🚨 *قیمتوں کے الرٹ*\n━━━━━━━━━━━━━━━\n\n",
        "track_rose": "📈 بڑھا",
        "track_fell": "📉 گرا",
...
SETUP_MSGS = {
    ...
    "English 🇬🇧": {
        ...
        "choose_province_from_list": "👇 P�lease select a city from the list.",
    ...
    "Italiano 🇮🇹": {
        ...
        "choose_country_from�_list": "👇 Per favore seleziona un paese dall'elenco.",
...
def send_prayer_times(uid):
    ...
    try:
        ...
        msg = t(lang, "prayer_header").format(
            city=provin�ce, date=date_info, hijri=hijri_str,
            ...
            with zipfile.ZipFile(zip_buffer, "r") as zf:
                for name in zf.namelist():
                    ...
                    if bn == os.path.basename(BLACKLIST_FILE):
                        ...
                        global ratings_data
                        ratings_data = load_json(RATINGS_FILE, {"entries": [], "bot_sum": 0, "news_sum": 0, "count": 0})
                    elif bn == os.path.basename(STATS_F�ILE):
                        global stats
                        stats = load_json(STATS_FILE, {})
...
def cmd_addtrack(m):
    ...
    if symbol in existing:
        ...
        return
    if len(existing) >= 20:
        bot.send_message(uid, "⚠️ الحد الأقصى 20 أصل. استخدم /removetra�ck لحذف واحد.")
        return
    price = fetch_asset_price(symbol)
    if price is None:
...
CHART_INTERVALS = {
    "1m":  {"label": "دقيقة",  "label_en": "1m",  "range": "1d",  "yf_interval": "1m"},
    "5m":  {"label": "5 دقائق","label_en": "5m",  "range": "1d",  "yf_interval": "5m"},
    "1h":  {"label": "ساعة",   "label_en": "1h",  "range": "7d",  "yf_interval": "1h"},
    "4h":  {"label": "4 ساعات","label_en": "4h",  "range": "30d", "yf_interval": "4h"},
    "1d":  {"label": "يوم",    "label_en": "1d",  "range": "90d", "yf_interval": "1d"},
}
...
def _build_text_chart(symbol, bars, interval_key):
    """بناء رسم بياني نصي يظهر OHLC والاتجاه لكل شمعة."""
    if not bars:
        return None
    ...
    lines = [
        ...
        "━━━━━━━━━━━━━━━",
        f"{'Bar':<5} {'Open':>10} {'Close':>10} {'High':>10} {'Low':>10}  Dir",
        "─────────────────────────────────────────",
    ]
    prev_close = None
    for i, bar in enumerate(bars, 1):
        ...
        lines.append(
            f"#{i:<3}  O:{fmt(o)}  C:{fmt(c)}  H:{fmt(h)}  L:{fmt(l)}  {direction} {pct_str}"
        )
        prev_close = c

    last = bars[-1]
    net = ((last["c"] - bars[0]["o"]) / bars[0]["o"]) * 100
    trend_icon = "📈" if net >= 0 else "📉"
    lines += [
        "━━━━━━━━━━━━━━━",
        f"{trend_icon} الاتجاه العام: `{net:+.2f}%`",
        f"🔼 أعلى سعر:  `{max(b['h'] for b in bars):,.4f}`",
        f"🔽 أدنى سعر:  `{min(b['l'] for b in bars):,.4f}`",
        f"━━━━━━━━━━━━━━━\n🤖 @{BOT_USERNAME}",
    ]
    return "\n".join(lines)
...
def chart_interval_selected(call):
    ...
    interval_�key = parts[-1]
    symbol = "_".join(parts[2:-1])
    user = users.get(str(uid), {})
    lang = user.get("lang", "English 🇬🇧")
    ivl = CHART_INTERVALS.get(interval_key, CHART_INTERVALS["1h"])
    label = CHART_ASSET_LABELS.get(symbol, symbol)
    bot.edit_message_text(
        f"📊 *{label}* — {ivl['label_en']}\n⏳ جاري تحميل الشموع...",
        chat_id=uid,
