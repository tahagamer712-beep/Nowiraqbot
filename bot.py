...
def help_cmd_callback(call):
    ...
    if cmd == "start":
        ...
        for row in CHART_ASSETS:
            buttons = [types.InlineKeyboardButton(CHART_ASSET_LABELS.get(s,s), callback_data=f"chart_asset_{s}") for s in row]
            markup.row(*buttons)
        bot.send_message(uid, prompt, reply_markup=markup)
    elif cmd == "news":
        if str(uid) in users:
...
def _addtrack_step(message):
    ...
    assets = tracked_assets[str(uid)].setdefault("assets", [])
    user = users.get(str(uid), {})
    lang = user.get("lang", "English 🇬🇧")
    if symbol not in assets:
        assets.append(symbol)
        save_tracked_assets()
        bot.send_message(uid, f"✅ تمت إضافة *{symbol}* لقائمة التتبع.", parse_mode="Markdown")
    else:
        bot.send_message(uid, f"⚠️ *{symbol}* موجود بالفعل.", parse_mode="Markdown")
...
def admin_callbacks(call):
    ...
    # ======== إدارة المستخدمين ========
        ...
        premium_list = stat�s.get("premium_users", [])
            ...
            try:
                ...
                for feed_url in feeds:
                    ...
                    for item in feed.entries[:50]:
                        ...
                        queue_send(chat_id, format_news_item(t(lang, "label_breaking"), title, lang),
                            parse_mode="Markdown", reply_markup=markup)
            except Exception:
                continue
        if changed:
...
def get_user_info(message):
    ...
    try:
        ...
        join_date = user.get("join_date", "غير معروف")
        interests = user.get("interests", [])
        notif = "✅ مفعّل" if use�r.get("notifications", True) else "❌ موقوف"
        track_data = tracked_assets.get(target_id, {})
        tracked = track_data.get("assets", [])
        msg = (
...
def search_news(uid, query):
    ...
    try:
        ...
        if not articles:
            ...
            return
        bot.se�nd_message(uid, t(lang, "search_results_header").format(query=query), parse_mode="Markdown")
            ...
            try:
                ...
                for feed_url in feeds:
                    ...
                    for item in feed.entries[:5]:
                        ...
                        item_sum = getattr(item, 'summary', '') or getattr(item, 'description', '')
                        markup = make_news_share_markup(link, title, lang, item_sum)
                        queue_send(uid, format_news_item(t(lang, "label_breaking"), title, lang),
                            parse_mode="Markdown", reply_markup=markup)
                if changed:
                       ...
                       � try:
                            ...
                            for item in feed_data.entries[:2]:
                                ...
                                if title and link:
                                    ...
                                    bot.send_message(uid,
                                        format_news_item(t(val, "label_news"), title, val),
                                        parse_mode="Markdown",
...
MSGS = {
    ...
    "Português 🇧🇷": {
        ...
        "forecast_7day_header": "🌤 *Previs�ão de 7 dias — {city}*\n\n",
   ...
   �     "forecast_error": "⚠️ Saatlik tahmin alınamıyor.",
    ...
    "Українська 🇺🇦": {
        ...
        "weekly_summary_text_header": "📆 *Тижн�еве зведення новин*\n📅 {start} — {end}\n━━━━━━━━━━━━━━\n",
    ...
    "Español 🇲🇽": {
        ...
        "prayer_error": "⚠️ No se pueden obt�ener horarios de oración ahora, inténtalo más tarde.",
...
SETUP_MSGS = {
    ...
    "English 🇬🇧": {
        ...
        "convert_send_both": "⚠️ Send the amount and curr�ency together. Example: *100 USD*",
...
# ======== دوال الأخبار ========
def format_news_item(prefix, title, lang=None):
    label = prefix
    return f"{label}\n\n📰 {title}{get_news_signature(lang)}"
    ...
    for feed_url in feeds:
        try:
            ...
            for item in feed.entries[:3]:
                ...
                bot.send_message(uid, format_news_item(t(lang, "label_news"), item.title, lang), parse_mode="Markdown", reply_markup=markup)
                count += 1
        except Exception as e:
    ...
    for feed_url in feeds:
        try:
            ...
            for item in feed.entries[:15]:
                ...
                item_sum = getattr(item, 'summary', '') or getattr(item, 'description', '')
                markup = make_news_share_markup(item.link, getattr(item, 'title', ''), lang, item_sum)
                bot.send_message(uid, format_news_item(t(lang, "label_breaking"), item.title, lang), parse_mode="Markdown", reply_markup=markup)
                count += 1
        except Exception as e:
            notify_admin_error(f"خطأ في RSS ({feed_url}): {e}")
    if count == 0:
    ...
    for feed_url in mena_feeds:
        try:
            ...
            for item in feed.entries[:10]:
                ...
                item_sum = getattr(item, 'summary', '') or getattr(item, 'description', '')
                markup = make_news_share_markup(link, title, lang, item_sum)
                bot.send_message(uid, format_news_item(t(lang, "label_mena"), title, lang), parse_mode="Markdown", reply_markup=markup)
            ...
            try:
                ...
                for item in feed.entries[:20]:
                    ...
                    if any(kw.lower() in title_lower for kw in MENA_KEYWORDS):
                        sent.add(link)
                        item_sum = getattr(item, 'summary', '') or getattr(item, 'description', '')
                        markup = make_news_share_markup(link, tit�le, lang, item_sum)
                        bot.send_message(uid, format_news_item(t(lang, "label_mena"), title, lang), parse_mode="Markdown", reply_markup=markup)
                        count += 1
                        if count >= 5:
                            break
...
MAX_NEWS_PER_BROADCAST = 3   # أقصى عدد أخبار لكل مستخدم في كل دورة بث
    ...
    for feed_url in feeds:
        try:
            ...
            for item in feed.entries[:5]:
                ...
                item_sum = getattr(item, 'summary', '') or getattr(item, 'description', '')
                markup = make_news_share_markup(item.link, getattr(item, 'title', ''), lang, item_sum)
                bot.send_message(uid, format_news_item(t(lang, "label_sports"), item.title, lang), parse_mode="Markdown", reply_markup=markup)
                count += 1
                if count >= 8:
                    break
        except Exception as e:
...
def send_prayer_times(uid):
    ...
    try:
        ...
        msg = t(lang, "prayer_header").format(
            city=province, date=date_inf�o, hijri=hijri_str,
...
def _build_backup_report():
    ...
    for idx, (uid_str, user) in enumerate(users.items(), 1):
        ...
        lines.append(f"  📍 المدينة/المحافظة: {province}")
    �    lines.append(f"  📅 تاريخ الانضمام: {join_date}")
        lines.append(f"  🕐 آخر ظهور: {last_seen}")
        lines.append(f"  🔔 الإشعارات: {notifications}")
        lines.append(f"  ⭐ مميز: {is_prem}")
        lines.append(f"  ⏰ انتهاء المميز المجاني: {ref_premium_expiry}")
        lines.append(f"  🚫 محظور: {is_ban}")
        lines.append(f"\n  📈 التتبع والتنبيهات:")
        lines.append(f"    • الرموز المتابعة (/mytrack): {tracked_symbols_str}")
        lines.append(f"    • المدن المتابعة (/weather): {cities_str}")
        ...
        lines.append(f"    • الميزات المفعّلة: {unlocked_str}")
        lines.append(f"    • عدد الأخبار المُرسلة: {sent_news_count}")
...
def cmd_addtrack(m):
    ...
    parts = m.text.strip().split()
    if len(parts) < 2:
        bot.send_message(uid, "⚠️ أرسل الرمز معه. مثال: `/addtrack BTC`", parse_mode="Markdown")
        return
    symbol = parts[1].upper()
    existing = tracked_assets.get(str(uid), {}).get("assets", [])
    if symbol in existing:
 ...
 �       return
    price = fetch_asset_price(symbol)
    if price is None:
        bot.send_message(uid, f"⚠️ لم يتم التعرف على الرمز *{symbol}*.", parse_mode="Markdown")
        return
    if str(uid) not in tracked_assets:
        tracked_assets[str(uid)] = {"assets": [], "last_prices": {}}
    tracked_assets[str(uid)]["assets"].append(symbol)
...
CHART_ASSETS = [
    ...
    ["USD", "EUR", "GBP", "IQD", "SAR"],
    ["GC=F", "SI=F", "CL=F", "BZ=F"],
    ["^GSPC", "^IXIC", "AAPL", "TSLA", "NVDA"],
]

CHART_ASSET_LABELS = {
    "GC=F": "Gold", "SI=F": "Silver", "CL=F": "WTI Oil",
    "BZ=F": "Brent Oil", "^GSPC": "S&P500", "^IXIC": "NASDAQ",
}
    ...
    for row in CHART_ASSETS:
        buttons = [
            types.InlineKeyboardButton(
                CHART_ASSET_LABELS.get(s, s),
                callback_data=f"chart_asset_{s}"
            ) for s in row
        ]
        markup.row(*buttons)
    bot.send_message(uid, prompt, reply_markup=markup)
