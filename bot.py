...
def _send_welcome_greeting(uid, name, lang="العربية 🇮🇶"):
...
def send_backup(chat_id=None):
    ...
    if not os.path.exists(DB_FILE):
        try:
            bot.send_message(target, "⚠️ ملف قاعدة البيانات غير موجود.")
        except Exception:
            pass
        return
    with _db_lock:
    ...
    try:
        ...
        try:
            bot.send_message(target, f"❌ فشل إرسال ملف .db: {e}")
        except Exception:
            pass

    # ── إرسال التقرير النصي المفصّل ──
    try:
        report = _build_backup_report()
        report_filename = f"backup_report_{file_now}.txt"
        encoded = report.encode("utf-8")
        import io
        report_file = io.BytesIO(encoded)
        report_file.name = report_filename
        bot.send_document(
            target,
            report_file,
            caption=(
                f"📋 *تقرير شامل بكل بيانات المستخدمين*\n"
                f"🕐 {now}\n"
                f"📄 يحتوي على تفاصيل كل مستخدم: معلوماته الأساسية، لغته، مدنه، رموزه المتابَعة، تنبيهاته، واشتراكه."
            ),
            visible_file_name=report_filename,
            parse_mode="Markdown"
        )
    except Exception as e:
        try:
            bot.send_message(target, f"❌ فشل إرسال التقرير النصي: {e}")
        except Exception:
            pass
...
def handle_document(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"📂 تم حفظ نسخة احتياطية للملف القديم: `{backup_path}`",
            parse_mode="Markdown"
        )
    except Exception as e:
