...
def send_referral_stats(uid):
    ...
    msg = (
        ...
        f"\n🔓 *ميزاتك المفتوحة:*\n{unlocked_names}\n"
        f"━━━━━━━━━━━━━━\n"
        f"💡 شارك رابطك وكلما زادت دعواتك زا�دت مكافآتك!"
    )
    markup = types.InlineKeyboardMarkup()
...
def _safe_job(fn):
    def wrapper():
        try:
            ...
            try:
                bot.send_message(AD�MIN_ID, f"\u26a0\ufe0f \u062e\u0637\u0623 \u0641\u064a \u0627\u0644\u0645\u0647\u0645\u0629 {fn.__name__}: {e}")
            except Exception:
                pass
...
def cmd_setsource(message):
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else None
    if not user_id or not is_chat_admin(chat_�id, user_id):
        return
