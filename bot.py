...
�        "track_rose": "📈 ارتفع",
    ...
    "English 🇬🇧": {
        ...
        "premium_btn_notif_ti�me": "🕐 Notification Time",
   ...
   �     "share_bot_header": "📢 *बॉट साझा करें और हमें बढ़ने में मदद करें!*\n\n🔗 *बॉट लिंक:*\n@{username}\n\n📌 या लिंक से:\n`{link}`\n\n━━━━━━━━━━━━━━━\n💡 दोस्तों के साथ साझा करें:",
    ...
    "اردو 🇵🇰": {
        ...
        "no_mena": "⚠️ ابھی کو�ئی نئی سیاسی خبر نہیں، بعد میں کوشش کریں۔",
...
SETUP_MSGS = {
    ...
    "Español 🇲🇽": {
        ...
        "notif_enable�d": "🔔 Las notificaciones automáticas han sido activadas.",
...
def _build_backup_report():
    ...
    for idx, (uid_str, user) in enumerate(users.items(), 1):
        ...
        telegram_lang = user.get("tel�egram_lang", "—")
...
def on_bot_chat_member_update(update):
    ...
    if new_status in ("administrator", "member") and old_status in ("left", "kicked", "restricted"):
        ...
        if not already:
            ...
            save_channels_groups()
        try:
            bot.send_message(chat_id, CHANNEL_WELCOME_MSG, pars�e_mode="Markdown")
        except Exception as e:
...
def cmd_pause(message):
    ...
    ch_data["paused"] = True
    save_channels_groups()
    bot.send_message(chat_i�d, "⏸ *تم إيقاف البث مؤقتاً.*\nاستخدم `/resume` للاستئناف.", parse_mode="Markdown")
...
@bot.m�essage_handler(commands=["chart"])
def cmd_chart(m):
    uid = m.from_user.id
...
def cb_settings_city(call):
    bot.answer_callba�ck_query(call.id)
    uid = call.from_user.id
