...
# PATCH: صلح RuntimeError للـ scheduler
# FIX: خُفِّض من 20 إلى 3 — 20 كان يسمح بـ 20 thread متوازية من الـ scheduler
try:
    from apscheduler.executors.pool import ThreadPoolExecutor as _APSTP
    _sch = getattr(_legacy, "scheduler", None)
    if _sch:
        _sch._executors["default"] = _APSTP(3)
        _sch._executors["default"].start(_sch, _sch._logger)
        _legacy._logger.info("✅ Scheduler executor تم تحديثه (max_workers=3)")
