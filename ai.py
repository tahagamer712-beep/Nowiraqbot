...
def **getitem**(self, k): return self._src()[k]
def **setitem**(self, k, v): self._src()[k] = v
def **delitem**(self, k): del self._src()[k]
def **contains**(self, k): return k in self._src()
def **iter**(self): return iter(self._src())
def **len**(self): return len(self._src())
...
now = time.time()
expired = [k for k, (*, t) in _AI_CACHE.items() if now - t > _AI_CACHE_TTL]
for k in expired:
...
_AI_SUMMARY_LOCK  = **import**(‘threading’).Lock()
...
if len(*AI_SUMMARY_CACHE) > 500:
now = time.time()
expired = [k for k, (*, t) in _AI_SUMMARY_CACHE.items() if now - t > _AI_SUMMARY_TTL]
for k in expired:
...
_recent_sent_titles[lang] = entries[-300:]   # الاحتفاظ بآخر 300
return False
