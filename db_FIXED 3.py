...
BACKUP_STATE_MSG_ID = _legacy.BACKUP_STATE_MSG_ID

# ════════════════════════════════════════════════════════════════════
# FIX: استيراد _FF من bot_legacy (يستخدم في _bc_save_checkpoint)
# ════════════════════════════════════════════════════════════════════
_FF = getattr(_legacy, "_FF", {})
_USER_SENT_TTL = getattr(_legacy, "_USER_SENT_TTL", 7 * 24 * 3600)
stats          = getattr(_legacy, "stats", {})
# ════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        for ch_data in ch_list:
            ...
            if "sent_news" in d:
                sn = d["sent_news"]
                if isinstance(sn, dict):
                    # احفظ الروابط النشطة فقط كـ list للـ DB
                    _now_db = time.time()
