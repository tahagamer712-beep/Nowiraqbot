...
# FIX B9/B10: استيراد types من telebot وADMIN_ID من _legacy
# كانت تُستخدَم مباشرةً بدون استيراد → NameError عند تشغيل أوامر الرياضة
from telebot import types  # noqa: E402  (B10: كانت تُستخدَم عبر `types.InlineKeyboardMarkup` بدون import)
...
bot         = _legacy.bot
# FIX B9: ADMIN_ID كانت تُستخدَم مباشرةً في دوال أوامر الرياضة (5 أماكن) دون استيراد
# → NameError يُوقف كل أوامر /tip و/admin المتعلقة بالرياضة بصمت
ADMIN_ID    = getattr(_legacy, 'ADMIN_ID', 0)
...
# FIX B6: _db_conn قد يتغير عند إعادة الاتصال بقاعدة البيانات.
# نستخدم proxy يقرأ دائماً من _legacy بدلاً من لقطة ثابتة.
class _DbConnProxy:
    """يُعيد توجيه كل استدعاء إلى _legacy._db_conn الحالي، مما يمنع مشاكل stale snapshot."""
    def __getattr__(self, name):
        return getattr(_legacy._db_conn, name)
    def execute(self, *a, **kw):        return _legacy._db_conn.execute(*a, **kw)
    def executemany(self, *a, **kw):    return _legacy._db_conn.executemany(*a, **kw)
    def executescript(self, *a, **kw):  return _legacy._db_conn.executescript(*a, **kw)
    def commit(self):                   return _legacy._db_conn.commit()
    def rollback(self):                 return _legacy._db_conn.rollback()
    def close(self):                    return _legacy._db_conn.close()

_db_conn               = _DbConnProxy()
...
# FIX B5: _AI_EXECUTOR قد يكون None إذا تم استيراد هذا الملف قبل أن يُصدِّر db.py
# مُنفِّذه. نُنشئ fallback حقيقي كما فعلنا في ai.py (Fix v14).
from concurrent.futures import ThreadPoolExecutor as _TPE_sp
_AI_EXECUTOR = getattr(_legacy, '_AI_EXECUTOR', None) or _TPE_sp(max_workers=4, thread_name_prefix="AIWorker_sp")
_ai_generate           = getattr(_legacy, '_ai_generate', lambda *a,**k: '')
# FIX B8: _safe_job كانت تُستخدم في تسجيل الـ scheduler (آخر الملف) بدون استيراد
# → NameError صامت يمنع تسجيل job التحديث التلقائي للنتائج الرياضية المباشرة.
_safe_job = getattr(_legacy, '_safe_job', lambda f: f)
...
# FIX B7: كان globals().get('scheduler') يبحث في namespace هذا الملف فقط،
# لكن `scheduler` موجود في bot_legacy.py (L28501) وليس هنا.
# النتيجة: _sched_ref = None دائماً → job التحديث لا يُسجَّل أبداً.
try:
    _sched_ref = getattr(_legacy, 'scheduler', None) or globals().get('scheduler')
